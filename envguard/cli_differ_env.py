...
def _render_text(result, source: str, target: str, show_unchanged: bool) -> None:
    """Render diff result as human-readable text to stdout."""
...
def _render_json(result, source: str, target: str, show_unchanged: bool) -> None:
    """Render diff result as JSON to stdout."""
    data = {
        "source": source,
        "target": target,
        "added": result.added,
...
def envdiff_cmd(source: str, target: str, fmt: str, strict: bool, show_unchanged: bool) -> None:
    ...
    if fmt == "json":
        _render_json(result, source, target, show_unchanged)
    else:
        _render_text(result, source, target, show_unchanged)
